...
// as other instruction can have a register, but addi needs at the last argument
// an immediate value.
uint64_t is_addi_instruction();

// Assignment 1 - Assembler Parser - Michael Lenort
// Is inherited from the -a flag when compiling selfie.
// Is a RISCU assembler parser to read and parse assembly instructions.
...
void copy_regs(uint64_t *forked_context,uint64_t *original_context);

void set_return_value_of_fork(uint64_t *forked_context,uint64_t *original_context);
...
uint64_t SYSCALL_PTHREAD_CREATE = 229;
uint64_t SYSCALL_PTHREAD_JOIN   = 230;
uint64_t SYSCALL_PTHREAD_EXIT   = 231;
...
uint64_t* add_context(uint64_t* context, uint64_t* from);
uint64_t* copy_context1(uint64_t* original);
uint64_t is_locked(uint64_t* context, uint64_t* from);

// Process id
uint64_t pid = 1;

void new_process_id() { pid = pid + 1; }
...
uint64_t process_id(uint64_t *context) {return (uint64_t) (context + 25);}
...
uint64_t get_pid(uint64_t *context) { return *(context + 25); }
...
void set_pid(uint64_t *context, uint64_t pid) { *(context + 25) = pid; }
...
uint64_t* zombie_contexts = (uint64_t*) 0; // doubly-linked list of zombie contexts
  ...
  if (find_next_character() != CHAR_EOF)
  {
    if (symbol != SYM_DIVISION)
    {
      ...
      else if (character == CHAR_DOT)
      {
        ...
        // we must change the scanner here to detect whetever
...
void emit_pthread_create() {
  create_symbol_table_entry(LIBRARY_TABLE, "pthread_create", 0, PROCEDURE, UINT64_T, 0, code_size);
...
void implement_pthread_create(uint64_t* context) {
  ...
  uint64_t* child_top;
  uint64_t vpaddr; //virtual page address
  uint64_t vindex; //virtual index or offset 
  uint64_t vcalc;

  child_context = copy_context1(context);
  
  vindex = get_page_of_virtual_address(get_code_seg_start(context));
  while (vindex < get_page_of_virtual_address(get_program_break(context)-8)+1) {
    if (is_page_mapped(get_pt(context), vindex))
      map_page(child_context, vindex, get_frame_for_page(get_pt(context), vindex));
    vindex = vindex + 1;
  }

  vindex = get_page_of_virtual_address(*(get_regs(context) + REG_SP));
  while (vindex < NUMBEROFPAGES) {
    vpaddr = 0;
    while (vpaddr < PAGESIZE) {
      vcalc = vindex*PAGESIZE+vpaddr;
      if (is_virtual_address_mapped(get_pt(context), vcalc)) {
        map_and_store(child_context, vcalc, load_virtual_memory(get_pt(context), vcalc));
      }
      vpaddr = vpaddr + 8;
    }
    vindex = vindex + 1;
  ...
  child_top = add_context(get_process_child(context), child_context);
  set_process_child(context, child_top);
  set_pid(child_context, pid);
  set_process_parent(child_context, context);
...
void emit_pthread_join() {
  create_symbol_table_entry(LIBRARY_TABLE, "pthread_join", 0, PROCEDURE, UINT64_T, 0, code_size);
...
void implement_pthread_join(uint64_t* context) {
  ...
  if (is_data_stack_heap_address(context, *(get_regs(context)+REG_A0))) {
    v = 1;
  } else if (*(get_regs(context)+REG_A0) == 0) {
    v = 2;
  } else {
    *(get_regs(context) + REG_A0) = -1; 
  }
  if (v > 0) {
    if (get_process_child(context) != (uint64_t*) 0) {
      forked_index = get_process_child(context);
      while(forked_index != (uint64_t*) 0) {
        if (get_process_state(get_process_parent(forked_index)) == STATUS_ZOMBIE) {
          zombie = get_process_parent(forked_index);
          set_process_state(zombie, STATUS_READY); 
          forked_index = (uint64_t*) 0;
        } else {
          forked_index = get_next_locked_context(forked_index);
        }
      }
      if (zombie == (uint64_t*) 0) {
        set_process_state(context, STATUS_BLOCK);
        locked_contexts = add_context(context, locked_contexts);      
        used_contexts = remove_context(context, used_contexts);
      } else {
        zombie_contexts = remove_context(zombie, zombie_contexts);
        if (v != 2) {
          map_and_store(get_parent(zombie), *(get_regs(get_parent(zombie))+REG_A0), left_shift(get_exit_code(zombie), 8));
        }
        // REG_A0, when parent waits, return PID from child
        *(get_regs(context) + REG_A0) = get_pid(zombie); 
      }
    } else {
    *(get_regs(context) + REG_A0) = -1;  //context has no children  
    }
  }
}

void emit_pthread_exit() {
  create_symbol_table_entry(LIBRARY_TABLE, "pthread_exit", 0, PROCEDURE, UINT64_T, 0, code_size);
...
void implement_fork(uint64_t *context) {
    ...
    forked_context = new_context();

    copy_regs(forked_context,context);
    copy_context(forked_context,context);

    set_lowest_lo_page(forked_context, 0);
    set_highest_lo_page(forked_context, get_lowest_lo_page(forked_context));
    set_lowest_hi_page(forked_context, get_page_of_virtual_address(HIGHESTVIRTUALADDRESS));
    set_highest_hi_page(forked_context, get_lowest_hi_page(forked_context));

    set_return_value_of_fork(forked_context,context);

    copy_memory(forked_context,context);

    set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
    set_pc(forked_context, get_pc(context));
...
void copy_regs(uint64_t *forked_context,uint64_t *original_context) {
    ...
    set_regs(forked_context, smalloc(NUMBEROFREGISTERS * WORDSIZE));

    while ( r < NUMBEROFREGISTERS) {
        *(get_regs(forked_context) + r) = *(get_regs(original_context) + r);
...
uint64_t* add_context(uint64_t* context, uint64_t* from) {
  set_prev_context(context, (uint64_t*) 0);
  set_next_context(context, from);

  if (from != (uint64_t*) 0) {
    set_prev_context(from, context);
  }
...
uint64_t* copy_context1(uint64_t* original) {
  ...
  set_highest_lo_page(context, get_page_of_virtual_address(get_program_break(original)));
...
void copy_context(uint64_t *forked_context,uint64_t *original_context) {
    ...
    set_code_seg_start(forked_context, get_code_seg_start(original_context));
    set_code_seg_size(forked_context, get_code_seg_size(original_context));
    set_data_seg_start(forked_context, get_data_seg_start(original_context));
    set_data_seg_size(forked_context, get_data_seg_size(original_context));
    set_heap_seg_start(forked_context, get_heap_seg_start(original_context));
    set_program_break(forked_context, get_program_break(original_context));
    set_process_parent(forked_context, original_context);
    new_process_id();
    set_pid(forked_context, pid);

    set_name(forked_context, get_name(original_context));

    set_pt(forked_context, zmalloc(NUMBEROFPAGES * SIZEOFUINT64STAR));
}

void copy_memory(uint64_t *forked_context,uint64_t *original_context) {
    uint64_t i;

    i = get_code_seg_start(original_context);

    while (i <= get_program_break(original_context)) {
        if(is_virtual_address_mapped(get_pt(original_context),i)) {
            map_and_store(forked_context, i, load_virtual_memory(get_pt(original_context), i));
        }

        i = i + WORDSIZE;
    }

    i = *(get_regs(original_context) + REG_SP);

    while (i <= HIGHESTVIRTUALADDRESS) {
        if(is_virtual_address_mapped(get_pt(original_context),i))
            map_and_store(forked_context,i, load_virtual_memory(get_pt(original_context),i));
...
void set_return_value_of_fork(uint64_t *forked_context,uint64_t *original_context) {
    ...
    *(get_regs(forked_context) + REG_A0)   = 0;
}
...
uint64_t print_code_context_for_instruction(uint64_t address)
{
  ...
  if (run)
  {
    w = dprintf(output_fd, "%s: pc==0x%lX", binary_name, address) + print_code_line_number_for_instruction(address, code_start);
    if (symbolic)
      // skip further output
      return w;
    else
...
uint64_t print_add_sub_mul_divu_remu_sltu()
{
  return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%s,%s", get_mnemonic(is), get_register_name(rd), get_register_name(rs1), get_register_name(rs2));
}
...
uint64_t print_load()
{
  return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%ld(%s)", get_mnemonic(is), get_register_name(rd), imm, get_register_name(rs1));
}
...
uint64_t print_store()
{
  return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%ld(%s)", get_mnemonic(is), get_register_name(rs2), imm, get_register_name(rs1));
}
...
uint64_t print_beq()
{
  ...
  w = print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%s,%ld", get_mnemonic(is), get_register_name(rs1), get_register_name(rs2), signed_division(imm, INSTRUCTIONSIZE));
...
uint64_t print_jal()
{
  ...
  w = print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%ld", get_mnemonic(is), get_register_name(rd), signed_division(imm, INSTRUCTIONSIZE));
  if (disassemble_verbose)
    return w + dprintf(output_fd, "[0x%lX]", pc + imm);
  else
    return w;
...
uint64_t print_jalr()
{
  return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%ld(%s)", get_mnemonic(is), get_register_name(rd), signed_division(imm, INSTRUCTIONSIZE), get_register_name(rs1));
}
...
uint64_t is_syscall_fork_or_exit() {
...
uint64_t is_syscall_thread_or_create() {
    if (*(registers + REG_A7) != SYSCALL_PTHREAD_CREATE)
        if (*(registers + REG_A7) != SYSCALL_PTHREAD_JOIN)
            return 1;
        else
            return 0;
    else
     ...
     if (is_syscall_fork_or_exit()){
            if(is_syscall_break_or_wait()) {
              ...
              if(is_syscall_lock_or_unlock()){
                if(is_syscall_thread_or_create()){
                read_register(REG_A1);
...
// all possible instructions given in the .s file format.

uint64_t is_ecall()
{
  if (identifier_string_match(SYM_ECALL))
    return 1;
  else
    return 0;
}
